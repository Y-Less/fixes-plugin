...
DECL_FIX(GetPlayerColor);

/*
 * FIXES_gscPlayerColours[100]
 *
 * There are only 100 colours used by default, with SA:MP looping through them
 * repeatedly.
 */
uint32_t
	FIXES(GetPlayerColor)::
	PlayerColours_[100] =
		{
...
{
	Log(LogLevel::DEBUG, "GetPlayerColor::OnPlayerConnect called");
	SetPlayerColor(playerid, GetDefaultColorForPlayer(playerid));
}

uint32_t
	FIXES(GetPlayerColor)::
	GetDefaultColorForPlayer(int playerid)
{
	return PlayerColours_[playerid % 100];
}
